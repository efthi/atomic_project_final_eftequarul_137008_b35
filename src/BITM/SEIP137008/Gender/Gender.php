...
class Gender extends DB
{
    ...
    public function index($mode='ASSOC'){
        $sql = "SELECT * FROM gender WHERE visible=1";
    ...
    public function trash(){
        $sql = "UPDATE gender SET visible= 0 WHERE id=".$this->id;
        $STH = $this->DBH->prepare($sql);
        $STH->execute();
        Utility::redirect('index.php');
    }
    ...
    public function trashitem($mode='ASSOC'){
        $sql = "SELECT * FROM gender WHERE visible=0";
        $STH = $this->DBH->query($sql);
    ...
    public function clear_trash(){
        $sql = "UPDATE gender SET visible= 1 WHERE id=".$this->id;
        $STH = $this->DBH->prepare($sql);
        $STH->execute();
        Utility::redirect('index.php');
    }
    ...
    public function indexPaginator($page=1,$itemsPerPage=5){
        ...
        $sql = "SELECT * from gender  WHERE visible ='1' LIMIT $start,$itemsPerPage";
    ...
    public function trashedPaginator($page=1,$itemsPerPage=5){
        ...
        $sql = "SELECT * from gender  WHERE visible ='0' LIMIT $start,$itemsPerPage";
    ...
    public function search($requestArray){
        $sql = "";
        if( isset($requestArray['byName']) && isset($requestArray['byGender']) )  $sql = "SELECT * FROM `gender` WHERE `visible` ='1' AND (`name` LIKE '%".$requestArray['search']."%' OR `gender` LIKE '%".$requestArray['search']."%')";
        if(isset($requestArray['byName']) && !isset($requestArray['byGender']) ) $sql = "SELECT * FROM `gender` WHERE `visible` ='1' AND `name` LIKE '%".$requestArray['search']."%'";
        if(!isset($requestArray['byName']) && isset($requestArray['byGender']) )  $sql = "SELECT * FROM `gender` WHERE `visible` ='1' AND `gender` LIKE '%".$requestArray['search']."%'";
